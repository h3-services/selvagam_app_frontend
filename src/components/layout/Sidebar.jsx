...
const Sidebar = () => {
  ...
  return (
    <div ref={sidebarRef}>
      ...
      <div
        className={`w-64 h-screen fixed lg:left-0 right-0 top-0 flex flex-col transition-transform duration-500 ease-out z-[10000] pointer-events-auto ${isOpen ? 'translate-x-0 rounded-bl-[2.5rem]' : 'translate-x-full'} lg:translate-x-0 lg:rounded-none`}
        style={{ 
      ...
      >
        {isOpen && (
          <button
            ...
            className="lg:hidden absolute top-4 left-4 z-[300] p-2 text-white hover:text-blue-200 transition-colors"
          >
            <RiCloseFill size={28} />
          </button>
        )}
